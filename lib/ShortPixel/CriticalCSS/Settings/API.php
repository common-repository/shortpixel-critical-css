...
class API extends Component {
	...
	public function admin_enqueue_scripts() {
		...
		wp_localize_script( 'ccss_admin_ui', 'ccssUILocal', [
			'forceWebCheck' => __( "The Critical CSS cache has been flushed and the Critical CSS generation will start again for each page when it's visited.", 'shortpixel-critical-css' ),
            'removeExpiredMessage' => __( "Expired entries older than 30 days have been removed from the processed log.", 'shortpixel-critical-css' ),
		] );
	        ...
	        $('.spccss_notice .dismiss-button').click( function(e){
		        ...
		        $.ajax( {
                    ...
                    data       : {
			            ...
			            spccssnonce: jQuery('#spccssnonce_usekey').val(),
                        causer : causer
                    },
    ...
    function enqueue_contact_form_scripts()
    {
	    ...
	    wp_localize_script( 'contact-form', 'ccssContactFormLocal', [
		    'description' => __( "We understand that this might feel complicated at first. The plugin team would be more than happy to help or to receive feed-back on how the plugin could be improved. \nJust fill in the form below and hit \"Send\" to get in touch with us.", 'shortpixel-critical-css'),
		    'name'        => __( "Your Name", 'shortpixel-critical-css' ),
		    'email'       => __( "Your E-mail" ),
		    'texthint'    => __( "Please let us know the domain name, PHP and WP version and any other information you think is relevant.", 'shortpixel-critical-css' ),
		    'send'        => __( "Send", 'shortpixel-critical-css' ),
		    'buttonTitle' => __( "Get help or send us feed-back.", 'shortpixel-critical-css' ),
		    'close'       => __( "Close", 'shortpixel-critical-css' ),
		    'buttonText'  => __( "Need help?", 'shortpixel-critical-css' ),
		    'formError'   => __( "Error sending the message. Please try again.", 'shortpixel-critical-css' ),
		    'formSuccess' => __( "Your message has been sent. Thank you!", 'shortpixel-critical-css' ),
	    ] );
	...
	public function callback_text( $args ) {
		...
		$value       = empty( $args['value'] )
                     ? esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) )
                     : $args['value'];
                        ...
                        $('.spccss-get').on('click', function(event) {
                            ...
                            $.ajax( {
                                ...
                                data       : {
                                    ...
                                    spccssnonce: jQuery('#spccssnonce').val(),
                                ...
                                success    : function( response ) {
                                    ...
                                    if(response.status == 'error') {
                                        modal.querySelector('.spccss-modal-css').innerHTML = response.response;
                                    } else {
                        ...
                        $('.spccss-api-action').on('click', function(event) {
                            ...
                            $.ajax( {
                                ...
                                success    : function( response ) {
                                    ...
                                    // all ::STATUS are constants not variables, so there will be no escape when echoe'd
                                    if(response.status == '<?php echo \ShortPixel\CriticalCSS\API::STATUS_DONE ?>') {
                                    ...
                                    } else if(response.status == '<?php echo \ShortPixel\CriticalCSS\Queue\Web\Check\Table::STATUS_DONE ?>' ||
                                        response.status == '<?php echo \ShortPixel\CriticalCSS\Queue\Web\Check\Table::STATUS_EXISTS ?>') {
                                        alert("Web check completed, added to the API Queue.");
                                        window.location.reload();
                                    }  else if(response.status == '<?php echo \ShortPixel\CriticalCSS\API::STATUS_QUEUED ?>') {
                                        alert("The job is queued for processing in the ShortPixel Critical CSS processing cloud (" + response.queue_id + ")");
                                    } else if(response.status == '<? echo \ShortPixel\CriticalCSS\Queue\Web\Check\Table::STATUS_PENDING ?>'||
                                        response.status == '<?php echo \ShortPixel\CriticalCSS\Queue\Web\Check\Table::STATUS_PROCESSING ?>'
                                    ) {
                                        alert("The job is pending web check before pushing to ShortPixel Critical CSS processing queue");
                                        //API::STATUS_REMOVED is a constant, not a variable
                                    } else if(response.status == '<?php echo \ShortPixel\CriticalCSS\API::STATUS_REMOVED ?>') {
