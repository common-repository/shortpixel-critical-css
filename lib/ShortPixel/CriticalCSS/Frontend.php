	...
	public function print_styles() {
        ...
        if ( ! get_query_var( 'nocache' ) && ! is_404() ) {
			...
			if ( ! empty( $cache ) ) {
                ...
                if(strpos( $cache, "\0") > 0) {
                    $len = strlen($cache);
                    $cache = str_replace("\0", '\0', $cache);
                    $corrected = (strlen($cache)  - $len ) . ' NUL byte(s)';
                }
                ?>
				<style id="criticalcss" data-no-minify="1" data-corrected="<?php echo $corrected; ?>"><?php echo $cache ?></style>
				<?php
			}
