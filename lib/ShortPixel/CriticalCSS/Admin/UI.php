...
class UI extends Component {
	...
	public function settings_init() {
		...
		wp_localize_script( 'sp_ccss_apikey', 'ccssApikeyLocal', [
			'descriptionRegister' => __( 'API Key for ShortPixel\'s Critical CSS. Get yours for free at <a href="https://ShortPixel.com/free-sign-up" target="_blank">ShortPixel.com</a>. If you use our <a href="https://wordpress.org/plugins/shortpixel-image-optimiser/" target="_blank">ShortPixel Image Optimization</a> plugin, you can use the same API Key.', 'shortpixel-critical-css' ),
			'descriptionLogin'    => __( 'API Key for ShortPixel\'s Critical CSS. Manage your key on <a href="https://ShortPixel.com/login" target="_blank">ShortPixel.com</a>', 'shortpixel-critical-css' ),
			'buttonText'          => __( 'Validate & save api key', 'shortpixel-critical-css' ),
		] );

        $api_key = $this->plugin->settings_manager->get_setting( 'apikey' );
		if ( is_multisite() ) {
			$hook = add_submenu_page( 'settings.php', 'ShortPixel Critical CSS Settings', 'SP Critical CSS', 'manage_network_options', CriticalCSS::LANG_DOMAIN, [
				$this,
				'settings_ui',
			] );
		} else {
			$hook = add_options_page( 'ShortPixel Critical CSS Settings', 'SP Critical CSS', 'manage_options', CriticalCSS::LANG_DOMAIN, [
				$this,
				'settings_ui',
			] );
		}
		...
		$this->settings_ui->add_section( [
			...
			'title' => __( 'Options', 'shortpixel-critical-css' ),
            'desc' => ( $api_key === ''
                ? __('<h4>Thank you for installing ShortPixel Critical CSS! To start generating critical CSS for your pages, you will need an API key. You can get it for free when you register at <a href="https://www.ShortPixel.com/free-sign-up" target="_blank">ShortPixel.com</a></h4>', 'shortpixel-critical-css') : '')
            . "<input type='hidden' name='spccssnonce' id='spccssnonce' value='" . wp_create_nonce( 'spccss-apikey' ) . "'>",
		] );
		...
		$this->settings_ui->add_field( $this->plugin->get_option_name(), [
			...
			'desc'              =>
                ($api_key === '')
                ? __( 'API Key for ShortPixel\'s Critical CSS. Get yours for free at <a href="https://ShortPixel.com/free-sign-up" target="_blank">ShortPixel.com</a>. If you use our <a href="https://wordpress.org/plugins/shortpixel-image-optimiser/" target="_blank">ShortPixel Image Optimization</a> plugin, you can use the same API Key.', 'shortpixel-critical-css' )
                : __( 'API Key for ShortPixel\'s Critical CSS. Manage your key on <a href="https://ShortPixel.com/login" target="_blank">ShortPixel.com</a>', 'shortpixel-critical-css'),
		] );
		if( $api_key === '' &&
...
//                    'type' => 'checkbox',
//                    'desc' => __('Force a web check on all pages for css changes. This will run for new web requests.', 'shortpixel-critical-css'),
//                ]);
//            }
            if ('on' !== $this->plugin->settings_manager->get_setting('prioritize_manual_css')) {
                $cacheModeOptions = [];
                $cacheModeOptions['postTypes'] = __('Based on Post Types', 'shortpixel-critical-css');

                if( !function_exists('ct_plugin_setup') ) { //oxygen builder plugin is not compatible with templates cache
                   $cacheModeOptions['templates'] = __('Based on WordPress templates', 'shortpixel-critical-css');
                }
                $cacheModeOptions['posts'] = __('Based on the individual URL', 'shortpixel-critical-css');
	            ...
	            $this->settings_ui->add_field($this->plugin->get_option_name(), [
                    ...
                    'desc' => __('<p style="font-style: italic;">At least one option should be selected. </p>', 'shortpixel-critical-css'),
                ]);
	            ...
	            $this->settings_ui->add_field($this->plugin->get_option_name(), [
		            ...
		            'type' => 'multicheck',
		            'options' => $postTypes,
		            //'desc' => __('Cache Critical CSS based on WordPress templates and not the post, page, term, author page, or arbitrary url. <p style="font-weight: bold;">This option may be useful if your website contains lots of content, pages, posts, etc. </p>', 'shortpixel-critical-css'),
	            ]);
            ...
            $this->settings_ui->add_field($this->plugin->get_option_name(), [
                ...
                'type' => 'checkbox',
                'desc' => __('Allow the CSS for a post, term, post type or taxonomy to always override the generated CSS. By default, the generated CSS takes precedence if it is present.', 'shortpixel-critical-css'),
            ]);
            ...
            if (!apply_filters('shortpixel_critical_css_cache_integration', false)) {
                $this->settings_ui->add_field($this->plugin->get_option_name(), [
                    ...
                    'type' => 'number',
                    'desc' => __('How often in seconds web pages should be checked for changes to regenerate the critical CSS for them.', 'shortpixel-critical-css'),
                ]);
            }
            $this->settings_ui->add_field($this->plugin->get_option_name(), [
                ...
                'type' => 'textarea',
                'desc' => __('A list of CSS selectors and/or regex patterns for CSS selectors', 'shortpixel-critical-css'),
                'sanitize_callback' => [
                    $this,
            ...
            $this->settings_ui->add_field($this->plugin->get_option_name(), [
                ...
                'type' => 'checkbox',
                'desc' => __('Load CSS files asynchronously to speed up the Largest Contentful Paint of your pages (Core Web Vital).', 'shortpixel-critical-css'),
            ]);
            if ('on' === $this->plugin->settings_manager->get_setting('prioritize_manual_css')) {
                foreach (get_post_types() as $post_type) {
                    ...
                    $this->settings_ui->add_field($this->plugin->get_option_name(), [
                        ...
                        'type' => 'checkbox',
                        'desc' => sprintf(__('Use a single CSS for all %s items', 'shortpixel-critical-css'), $label),
                    ]);
                    if ('on' === $this->plugin->settings_manager->get_setting("single_post_type_css_{$post_type}")) {
                        $this->settings_ui->add_field($this->plugin->get_option_name(), [
                            ...
                            'type' => 'textarea',
                            'desc' => sprintf(__('Enter CSS for all %s items', 'shortpixel-critical-css'), $label),
                        ]);
                        $this->settings_ui->add_field($this->plugin->get_option_name(), [
                            'name' => "single_post_type_css_{$post_type}_archive_css",
                            'label' => $label . ' post type archive CSS input',
                            'type' => 'textarea',
                            'desc' => sprintf(__('Enter CSS for %s archive listings', 'shortpixel-critical-css'), $label),
                        ]);
                    }
                }
    ...
    public function spccss_customize_register( $wp_customize ) {
        ...
        $wp_customize->add_control( new \WP_Customize_Code_Editor_Control (
            ...
            array(
                'label'       => __( 'Fallback Critical CSS', 'shortpixel-critical-css'),
                'section'     => 'custom_css',
	...
	public function settings_ui() {
        echo('<div class="wrap spccss-settings"><h1>' . __('ShortPixel Critical CSS Settings', 'shortpixel-critical-css') . '</h1>');
		...
		if ( '' !== $this->plugin->settings_manager->get_setting( 'apikey' ) ||
			...
			<style type="text/css">
				...
				<?php ob_start(); ?>
				<p>
					<?php esc_html_e( 'This queue is designed to process your content only when the cache mode based on the URL is used. It detects changes to the content and sends them to the "API Queue" when they are found.', 'shortpixel-critical-css' ); ?>
				</p>
                <?php if( !$this->plugin->settings_manager->get_setting('loopback_available') ): ?>
                    <div class="error" style="text-align: left; margin: 10px 0;">
                        <p>
                            <?php esc_html_e( 'The Web check queue may not work as intended because the web loop-back call failed. Please check your server configuration to make sure >> wp_remote_get( home_url()) is allowed. <<', 'shortpixel-critical-css' ); ?>
                        </p>
                    </div>
                <?php endif; ?>
				<form method="post">
			...
			ob_start(); ?>
			<p>
				<?php esc_html_e( 'This queue processes requests by sending them to ShortPixel Critical CSS and waiting for them to be processed. When it\'s done, the supported cache is flushed and the page gets faster. :)', 'shortpixel-critical-css' ); ?>
			</p>
			<form method="post">
				<?php
			...
			ob_start(); ?>
			<p>
				<?php esc_html_e( 'This is a list of all pages, links, and/or templates that have already been processed.', 'shortpixel-critical-css' ); ?>

            <!-- Remove EXPIRED Button -->
            <?php _e(
                '
                <input type="submit" name="spccss_removexp" id="spccss_removexp" class="button button-primary"
                       title="Remove expired items older than 30 days" value="Remove Expired" onclick="removeExpiredResults()">   
                ',
                'shortpixel-critical-css'
            ); ?>
            </p>
            <script>
                function removeExpiredResults() {
                    const remExp = new XMLHttpRequest();
                    remExp.open('POST', ajaxurl, true);
                    remExp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                    remExp.onload = function () {
                        if (remExp.status === 200) {
                            const removeExpiredMessage = ccssUILocal.removeExpiredMessage;
                            const successMessage = `<div id="remove-success-message" class="notice notice-success is-dismissible"><p>${removeExpiredMessage}</p></div>`;
                            jQuery('h2.nav-tab-wrapper').before(successMessage);
                            setTimeout(function() {
                                jQuery('#remove-success-message').fadeOut('slow', function() {
                                    jQuery(this).remove();
                    ...
                    remExp.send('action=remove_expired_results');
                }
            </script>
