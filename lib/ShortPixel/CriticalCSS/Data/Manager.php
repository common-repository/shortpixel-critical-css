		...
		switch ( $item['type'] ) {
			case 'post':
				...
				if(get_post_type_object($post_type) === null){
					//(SPCCSS_DEBUG & FileLog::DEBUG_AREA_INIT) && FileLog::instance()->log("Null object for item: ", $item['object_id'] . ' \n unique message post type: ' . $post_type);
					$hierarchical = false;
				} else {
			...
			case 'term':
				...
				if (!is_wp_error($term) && is_object($term)) {
					...
					if ($taxonomy && is_object($taxonomy)) {
						...
						//(SPCCSS_DEBUG & FileLog::DEBUG_AREA_INIT) && FileLog::instance()->log("Invalid taxonomy for term: ", $item['object_id']);
						$hierarchical = false;
					}
				} else {
					$hierarchical = false;
