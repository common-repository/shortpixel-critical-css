...
class CriticalCSS extends Plugin {
	 ...
	 *
	 */
	const LANG_DOMAIN = 'shortpixel-critical-css';
	...
	public function get_permalink(	array $object ) {
		...
		switch ( $object['type'] ) {
			...
			case 'term':
				...
				$term_id = $object['object_id'];
				$url = get_term_link($term_id);
				// handle WP_Error
				if ($url instanceof \WP_Error ) {
					return false;
//					if (defined('SPCCSS_DEBUG') && SPCCSS_DEBUG & FileLog::DEBUG_AREA_INIT) {
//						FileLog::instance()->log("Invalid taxonomy for term: ", $term_id);
//					}
				} elseif (!$url) {
					// handle bool return
//					if (defined('SPCCSS_DEBUG') && SPCCSS_DEBUG & FileLog::DEBUG_AREA_INIT) {
//						FileLog::instance()->log("Failed to get term link for term ID: ", $term_id);
//					}
					return false;
				}
				break;
	...
	public function web_queue_run(int $queueId) {
		$nonce  = isset($_POST['spccssnonce']) ? sanitize_text_field($_POST['spccssnonce']) : '';
		if ( ! wp_verify_nonce($nonce, 'spccss-apikey') ) {
			$ret = [
				'status' => 'error'
			];
	...
	protected function run($dataSource, $queue, $queueId) {
		...
		if($rec === null) {
			exit(wp_send_json(["status" => API::STATUS_DONE]));
		}
	...
	protected function queue_remove($queue, int $queueId)
	{
		...
		echo wp_send_json( [ 'status' => API::STATUS_REMOVED ] );//API::STATUS_REMOVED is a constant, not a variable
		exit;
    ...
    public function get_ccss() {
	    $nonce  = $_POST['spccssnonce'];
	    if ( ! wp_verify_nonce( $nonce, 'spccss-apikey' ) ) {
		    $ret = [
			    'status' => 'error',
			    'response' => 'The link you followed has expired. <a href="options-general.php?page=shortpixel_critical_css">Please try again</a>.'
		    ];
		   // header('Content-Type: application/json; charset=utf-8');
		    echo wp_send_json( $ret );
		    exit;
	...
	public function update_apikey() {
		$key = sanitize_key(trim( $_POST['key'] ));
        $nonce  = isset($_POST['spccssnonce']) ? sanitize_text_field($_POST['spccssnonce']) : '';
        if ( ! wp_verify_nonce( $nonce, 'spccss-apikey' ) ) {
            $ret = [
                'status' => 'error',
                'error' => 'The link you followed has expired. <a href="options-general.php?page=shortpixel_critical_css">Please try again</a>.'
            ];
           // header('Content-Type: application/json; charset=utf-8');
            echo wp_send_json($ret);
            exit();
	...
	public function dismiss_notification() {
		$causer = sanitize_text_field(! empty( $_POST['causer'] ) ? $_POST['causer'] : null);
		$nonce  = $_POST['spccssnonce'];
		if ( ! wp_verify_nonce( $nonce, 'spccss-apikey' ) ) {
			$ret = [
				'status' => 'error',
